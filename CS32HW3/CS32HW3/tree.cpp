...
int countIncludes(const string a1[], int n1, const string a2[], int n2)
{
    ...
    if (n2 <= 0) //no elements in a2
    {
        return 1;
    }
    if (n1 <=0) //a1 is 0 or less
    {
        return 0;
    }
    int counter = 0;
    if (a1[0] == a2[0]) //if theres match
    {
        counter+=countIncludes(a1+1, n1 - 1, a2+1, n2 - 1);
    }
    counter+=countIncludes(a1+1, n1 - 1, a2, n2); //next element
    return counter;
...
void order(string a[], int n)
{
    int firstNotLess;
    int firstGreater;
    if(n>1)// one or less elements returns function
    {
        separate(a, n, a[n-1], firstNotLess, firstGreater);
        order(a,firstNotLess);
        order(a+firstNotLess, n-firstNotLess);
    }
}
